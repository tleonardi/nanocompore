...
def paired_test (ref_pos_dict, method="mann_whitney", sequence_context=0, min_coverage=20):
    ...
    for pos, pos_dict in ref_pos_dict.items():
        ...
        # ref_pos_dict[pos]["median_pvalue"] = stat_test (pos_dict["S1_median"], pos_dict["S2_median"])[1]
        # ref_pos_dict[pos]["dwell_pvalue"] = stat_test (pos_dict["S1_dwell"], pos_dict["S2_dwell"])[1]
    ...
    if sequence_context:
        for mid_pos in ref_pos_dict.keys():
            ...
            try:
                ...
                ref_pos_dict[mid_pos]["pvalue_sliding_median"] = combine_pvalues (pval_median_list, method='fisher') [1]
                ref_pos_dict[mid_pos]["pvalue_sliding_dwell"] = combine_pvalues (pval_dwell_list, method='fisher') [1]
