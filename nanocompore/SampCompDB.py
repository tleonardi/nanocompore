...
class SampCompDB(object):
    ...
    def plot_pvalue( self,
        ...
        * tests
        ...
        with pl.style.context(plot_style):
            ...
            if kind == "scatterplot":
                _ = sns.scatterplot(data=df, palette=palette, ax=ax, s=200)
                if end-start<30:
            ...
            _ = ax.legend(bbox_to_anchor=(1.01, 0.5), loc=2, facecolor="white", frameon=False, fontsize=14)
            _ = ax.set_ylabel("-log (pvalue)")
            _ = ax.set_xlabel("Reference position")
            _ = ax.set_title(ref_id)
            _ = ax.set_facecolor('white')
            _ = ax.xaxis.label.set_color('black')
            _ = ax.yaxis.label.set_color('black')
            #_ = ax.grid(True, color='grey')
            _ = ax.tick_params(which='both', bottom=True, top=False,
                               right=False, left=True, labelsize=10,
                               color='black', labelcolor='black')
            _ = ax.spines['bottom'].set_color('black')
            _ = ax.spines['top'].set_color('black')
            _ = ax.spines['right'].set_color('black')
    ...
    def plot_position(self,
        ...
        * gmm_levels
        ...
        # set cal_type for now
        calc_type = ['dwell', 'prev_dwell', 'variance']
        ...
        for cond_lab, cond_dict in ref_data[pos]["data"].items():
            if split_samples:
                for samp_lab, sample_val in cond_dict.items():
                    if 'prev_dwell' in calc_type:
                        cintensity = []
                        cdwell = []
                        cprev_dwell = []
                        cvariance = []
                        for i,d,p,v in zip(sample_val["intensity"], sample_val["dwell"], sample_val["prev_dwell"], sample_val["variance"]):
                            if p != 1000.0:
                                cintensity.append(i)
                                cdwell.append(np.log10(d))
                                cprev_dwell.append(np.log10(p))
                                cvariance.append(v)
                        if len(cintensity) == 0:
                            cintensity = sample_val["intensity"]
                            cdwell = np.log10(sample_val["dwell"])
                            cvariance = sample_val["variance"]
                            cprev_dwell = [0]*len(sample_val["intensity"])
                    else:
                        cintensity = sample_val["intensity"]
                        cdwell = np.log10(sample_val["dwell"])
                        cprev_dwell = np.log10(sample_val["prev_dwell"])
                        cvariance = cvariance = sample_val["variance"]

                    plot_data_dict["{}_{}".format(cond_lab, samp_lab)] = {
                        "intensity":scale(cintensity),
                        "dwell":scale(cdwell),
                        "prev_dwell":scale(cprev_dwell),
                        "variance":scale(cvariance),
                        "color":next(col_gen)}
            else:
                intensity_list = []
                dwell_list = []
                ...
                for samp_lab, sample_val in cond_dict.items():
                    ...
                    prev_dwell_list.append(sample_val["prev_dwell"])
                    variance_list.append(sample_val["variance"])
                ...
                if 'prev_dwell' in calc_type:
                    ...
                    cprev_dwell = []
                    cvariance = []
                    for i,d,p,v in zip(np.concatenate(intensity_list), np.concatenate(dwell_list), np.concatenate(variance_list), np.concatenate(prev_dwell_list)):
                        if p != 1000.0:
                            cintensity.append(i)
                            cdwell.append(np.log10(d))
                            cprev_dwell.append(np.log10(p))
                    ...
                    if len(cintensity) == 0:
                        cintensity = np.concatenate(intensity_list)
                        cdwell = np.log10(np.concatenate(dwell_list))
                        cvariance = np.concatenate(variance_list)
                        cprev_dwell = [0]*len(np.concatenate(intensity_list))
                else:
                    cintensity = np.concatenate(intensity_list)
                    cdwell = np.log10(np.concatenate(dwell_list))
                    cprev_dwell = np.log10(np.concatenate(prev_dwell_list))
                    cvariance = cvariance = np.concatenate(variance_list)
        ...
        if model and 'txComp' in ref_data[pos] and 'GMM_model' in ref_data[pos]['txComp']:
            ...
            global_variance = scale(np.concatenate(([v['variance'] for v in data[condition_labels[0]].values()]+[v['variance'] for v in data[condition_labels[1]].values()]), axis=None))
            global_prev_dwell = scale(np.log10(np.concatenate(([v['prev_dwell'] for v in data[condition_labels[0]].values()]+[v['prev_dwell'] for v in data[condition_labels[1]].values()]), axis=None)))

            x = np.linspace(min(global_intensity), max(global_intensity), num=1000)
            y = np.linspace(min(global_dwell), max(global_dwell), num=1000)
            y2 = np.linspace(min(global_prev_dwell), max(global_prev_dwell), num=1000)
            X, Y = np.meshgrid(x, y)
        ...
        # plot collected data
        with pl.style.context(plot_style):
            fig, axes = pl.subplots(nrows=len(calc_type), ncols=1, figsize=figsize, constrained_layout=True)

            for label, d in plot_data_dict.items():
                if kde:
                    for n, component in enumerate(calc_type):
                        _ = sns.kdeplot(
                            x=d["intensity"],
                            y=d[component],
                            cmap=sns.light_palette(d["color"], as_cmap=True),
                            ax=axes[n],
                            clip=((min(d["intensity"]), max(d["intensity"])), (min(d[component]),max(d[component]))))
            ...
            # Adjust display
            _ = pl.suptitle("%s\n%s (%s)"%(ref_id.replace('_', ' '),pos, ref_kmer))
            _ = axes[0].set_ylabel("log10 (Dwell Time)")
            _ = axes[1].set_ylabel("log10 (Prev Dwell Time)")
            _ = axes[2].set_ylabel("Variance")
            for ax, coponent in zip(axes, calc_type):
                #_ = axes[n].set_ylabel(component)
                _ = ax.set_xlabel("Median Intensity")
                _ = ax.set_xlim(xlim)
                _ = ax.set_ylim(ylim)
                _ = ax.legend(loc='center right', facecolor='white', fancybox=True, fontsize=10,
