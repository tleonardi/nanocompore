...
class Transcript_Data():
    def __init__(self, tx_name, reference_samples, test_samples, tx_length, max_tx_coverage, min_tx_coverage):
        ...
        self._all_condition_labels = ['reference', 'test']
    ...
    def getKmerSeq(self, pos):
        seqs = []
        for sample in self._reference_samples + self._test_samples:
            seqs.append(sample.getKmerSeq(pos))
    ...
    def getConditionLabels(self):
        return self._all_condition_labels
    
    def getSampleLabels(self):
        return self._all_sample_labels

    def getSample2Condition(self):
        sample2condition = {}
        for sample in self._reference_samples + self._test_samples:
            sample2condition[sample.sample_label] = sample.condition_label
        return sample2condition
