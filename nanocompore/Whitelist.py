...
# Local package
from nanocompore.helper_lib import counter_to_str, access_file
...
class Whitelist (object):
    ...
    def _read_eventalign_index (self):
        ...
        for lab, fn in ("S1", self.__s1_index_fn), ("S2", self.__s2_index_fn):
            with open (fn) as fp:
                ...
                for field in ["ref_id", "ref_start", "ref_end", "read_id", "kmers", "NNNNN_kmers", "mismatching_kmers", "missing_kmers", "offset"]:
    ...
    def _select_intervals (self, ref_reads):
        ...
        for ref_id, sample_reads in tqdm (ref_reads.items ()):
            ...
            if valid_interval_list:
                ...
                # Save valid intervals and all reads mapped on reference
                ref_interval_reads [ref_id] = {
                    "interval_list": valid_interval_list,
                    "S1": sample_reads["S1"],
                    "S2": sample_reads["S2"]}
