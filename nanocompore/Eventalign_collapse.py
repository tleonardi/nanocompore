        ...
        try:
            ...
            for events_l in iter(in_q.get, None):
                ...
                for event_d in events_l:
                    read.add_event(event_d, events_l)
                    n_events+=1
...
class Read ():
    ...
    def add_event (self, event_d, events_l):
        self.n_events+=1
        ...
        # Update current kmer
        self.kmer_l[-1].add_event(event_d, events_l)
...
class Kmer ():
    ...
    def add_event (self, event_d, events_l):
        ...
        #Logan trying to add previous dwell time information
        ref_poss = []
        for event_d0 in events_l:
            ref_poss.append(int(event_d0["ref_pos"]))
        if int(self.ref_pos) + 11 in ref_poss:
            if self.prev_dwell == 0.0:
                for event_d2 in events_l:
                    if event_d2["ref_pos"] == int(self.ref_pos) + 11:
                        self.prev_dwell += event_d2["dwell_time"]
        else:
            self.prev_dwell = 1000.0
    ...
    def get_results(self):
        ...
        #logan added vairance
        d["variance"] = np.var(sample_array)
        d["prev_dwell"] = self.prev_dwell
