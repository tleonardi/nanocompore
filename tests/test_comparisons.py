...
def test_kmers_to_tensor():
    ...
    expected = torch.tensor([
            # pos 3
            [
                [97.5, np.log10(0.08)], # read 1
                [98.8, np.log10(0.012)], # read 2
                [84.3, np.log10(0.4)], # read 3
                [113.2, np.log10(0.06)], # read 4
                [np.nan, np.nan], # read 5
                [np.nan, np.nan] # read 6
            ],
            # pos 5
            [
                [98.5, np.log10(0.09)], # read 1
                [99.8, np.log10(0.013)], # read 2
                [85.3, np.log10(0.5)], # read 3
                [114.2, np.log10(0.07)], # read 4
                [np.nan, np.nan], # read 5
                [np.nan, np.nan] # read 6
            ],
            [
                [82.3, np.log10(0.3)], # read 1
                [111.2, np.log10(0.03)], # read 2
                [np.nan, np.nan], # read 3
                [np.nan, np.nan], # read 4
                [96.5, np.log10(0.07)], # read 5
                [97.8, np.log10(0.01)] # read 6
            ]
        ], dtype=torch.float32)
...
def test_kmers_to_tensor_with_motor_dwell():
    config_yaml = copy.deepcopy(BASIC_CONFIG)
    config_yaml['motor_dwell_offset'] = 11
    config = Config(config_yaml)
    comparator = TranscriptComparator(config)
